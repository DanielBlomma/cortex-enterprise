...
test("admin can access all actions", () => {
  const actions = [
    "policy.write", "policy.sync", "telemetry.configure", "license.manage",
    "audit.query", "policy.list", "telemetry.status",
    "enterprise.status", "license.status",
  ];
...
test("developer can access developer+all actions", () => {
  const allowed = ["audit.query", "policy.list", "telemetry.status", "enterprise.status", "license.status"];
...
test("developer cannot access admin-only actions", () => {
  const denied = ["policy.write", "policy.sync", "telemetry.configure", "license.manage"];
...
test("readonly can only access all-role actions", () => {
  const allowed = ["enterprise.status", "license.status"];
  const denied = [
    "policy.write", "policy.sync", "telemetry.configure", "license.manage",
    "audit.query", "policy.list", "telemetry.status",
  ];
